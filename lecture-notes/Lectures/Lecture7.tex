...
\subsection{Introduction to Charging Effects}

\begin{itemize}
    \item Charging effects (\ylink{0:18})
...
\subsection{Capacitance and Energy Scales}

\begin{itemize}
    \item If charging energy is not available from external voltage sources, T, etc, then transport is blocked (\ylink{3:41}).
    ...
    \item Mesoscopic systems, typically C in 1-100 fF range. Ec in 0.5-50meV range (\ylink{4:09}).
...
\subsection{Conditions for Coulomb Blockade}

\begin{itemize}
    \item Conditions for observing Coulomb blockade (\ylink{12:30}):
          \begin{itemize}
...
\subsection{Single Electron Transistor (SET)}
...
\begin{itemize}
    ...
    \item Current flows if a dot chemical potential lies inside the bias window (window between source and drain chemical potentials) (\ylink{24:43}, \ylink{28:03}).
...
\subsection{Historical Development and Fabrication}
...
\subsection{Coulomb Diamonds and Stability Diagrams}

\begin{itemize}
    \item Coulomb diamonds: the lines define a region in which there is no current. This region is called the coulomb diamond. At zero bias, current flows at the blue degeneract points (\ylink{34:51}).
          \begin{figure*}[h!]
              \centering
    ...
    \item Slope of lines tells you about the coupling capacitancebetween the source/drain/gate and the island (\ylink{37:05}).
    ...
    \item A low-bias gate trace shows peaks in conductance each time the number of electrons on the dot changes by one (\ylink{41:21}).
    \item Asymmetric coupling: Coulomb staircase (Matsumoto et al. Appl. Phys. Lett. 68, 34, 1996).
    \item Paper example with coulomb diamond: \href{https://pubs.aip.org/aip/apl/article-abstract/83/10/2052/115649/Single-electron-transistors-in-heterostructure}{"Single-electron transistors in heterostructure nanowires" by C. Thelander et al, Appl. Phys. Lett. 83, 2052 (2003) }
\end{itemize}

\subsection{Temperature Effects and Thermometry}

\begin{itemize}
    \item Temperature effects: as temperature increases, the peaks become oscillations until they eventually disappear.
    \item FWHM of a conductance peak can be used to estimate the electron temperature in the device. There is a quantum and classical equation for this (\ylink{51:12}).
    \item Remember: electron temperature does not need to equal the fridge temperature (insufficient electron-phonon coupling). Must use wire design and filtering to reduce noise and heating and cool electrons.
\end{itemize}

\subsection{Charge Sensing}

\begin{itemize}
    \item Paper shows off-set charges in coulomb diamond diagram: \href{https://pubs.aip.org/aip/apl/article-abstract/84/16/3154/523467/Metal-nanoparticle-single-electron-transistors?redirectedFrom=fulltext}{"Metal-nanoparticle single electron transistors fabricated using electromigration" by K.I. Bolotin et al, Appl. Phys. Lett. 84, 3154 (2004)}
    \item Can get random sudden offset charges in the environment that shift the coulomb diamonds.
    \item This is paves the way to charge sensing.
    \item QPC (or another QD or SET) can be used as detector for charge in quantum dot.
    \item Tune QPC to slope between conductance plateaus. Change in charge on dot changes potential landscape and hence conductance of QPC (\ylink{59:38}).
\end{itemize}

\subsection{Applications of SETs}

\begin{itemize}
    \item SET applications: sensitive charge ($10^{-5} e/\sqrt{Hz}$), readout of charge qubits, readout of spin qubits (via spin-to-charge conversion), current standard (turnstile)
    \item Single-electron logic and memory devices (\href{https://intra.ece.ucr.edu/~korotkov/papers/IJE-86-511-1999.pdf}{Alexander Korotkov 1999}). Issues: random offset charges, and room temp operation.
    \item Single-electron turnstile: AC pulse gates at fixed frequency to get current standard $I = ef$. NIST standard: 1 part in $10^8$ which is better than what the turnstiles can do (\ylink{1:02:26}).
    \item Room temp silicon SET (coulomb diamonds not very reproducible so can't be used in our normal computers): \href{https://pubs.aip.org/aip/apl/article-abstract/72/10/1205/518699/Silicon-single-electron-quantum-dot-transistor?redirectedFrom=fulltext}{Silicon single-electron quantum-dot transistor switch operating at room temperature by Lei Zhang et al, Appl. Phys. Lett. 1998}
\end{itemize}

\subsection{Cotunneling}

\begin{itemize}
    \item Cotunneling (2nd order perturbation theory): electron tunnels from source to higher level in dot, then to drain (\ylink{1:10:00}).
    \item See \href{https://journals.aps.org/prb/abstract/10.1103/PhysRevB.71.153402}{S. Sapmaz et al, Phys. Rev. B 71, 153402 (2005)} for cotunneling in carbon nanotube quantum dots.
    \item See \href{https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.93.256801}{D. Zumbuhl et al, PRL 93, 256801 (2004)} for cotunneling in GaAs quantum dots.
\end{itemize}